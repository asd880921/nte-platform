...
> 執行期間能不能碰電腦，看腳本的**執行模式**（見下方「內建腳本」）。預設都是 ⚪ **前台**：請保持遊戲在最上層，並且不要同時手動操作滑鼠鍵盤——腳本是用真實鍵鼠操作的，會互相打架。
>
...
| 腳本 | 支援模式 | 說明 |
| --- | --- | --- |
| 🍅 1-1 店長精選 | ⚪ 前台 | 自動重複刷取店長精選關卡。目前僅支援使用「安魂曲」角色。 |
| 🎣 自動釣魚 | ⚪ 前台 🟠 後台 | 自動循環釣魚。請先進入釣魚畫面按下「開始釣魚」後，再按 `F1` 啟動腳本。 |
...
| 模式 | 說明 | 限制 |
| --- | --- | --- |
| ⚪ 前台 | 使用全域鍵盤 / 滑鼠輸入，等同玩家實際操作，輸入會送至目前最上層視窗。 | 遊戲需保持在最上層，執行期間避免操作鍵盤滑鼠。 |
| 🟠 後台 | 使用 Windows 視窗訊息（`WM_KEYDOWN` / `WM_KEYUP`）直接傳送按鍵至遊戲視窗，並告知該視窗為作用中，不佔用實體輸入。 | 遊戲不可最小化，但可以被其他視窗覆蓋。 |
