...
工具列的齒輪按鈕可設定所有腳本共用的開始與停止快捷鍵，支援 `F1` 至 `F12`，兩個動作不可使用同一顆按鍵。預設為 `F1` 開始、`F2` 停止。

修改前請先停止執行中的腳本；儲存後，腳本卡片與待機訊息會同步顯示新的快捷鍵。
...
1. 將遊戲解析度設為 **1920 × 1080**，並進入「九百九十九夜」副本。
2. 使用前台模式啟動腳本，再按開始快捷鍵開始循環。
