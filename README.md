...
> 執行期間能不能碰電腦，看腳本標示的**模式**（見下方「內建腳本」）。目前內建的兩支都是 🟠 **前台**：請保持遊戲在最上層，並且不要同時手動操作滑鼠鍵盤——腳本是用真實鍵鼠操作的，會互相打架。
>
...
| 🎣 自動釣魚 | 🟠 前台 | 自動循環釣魚。請先進入釣魚畫面按下「開始釣魚」後，再按 `F1` 啟動腳本。 |

**模式代表執行期間你能做什麼**——面板上每張卡片的名稱旁都會標示，滑鼠移上去可看到說明：

- 🟠 **前台**：遊戲視窗需保持在最上層；執行期間請勿操作鍵盤滑鼠。
- 🔵 **後台**：遊戲視窗不可最小化，但可以被其他視窗蓋住；執行期間鍵盤滑鼠可自由使用，不影響腳本。
