...
- **自訂快捷鍵**：開始與停止按鍵可在 `F1` 至 `F12` 間自由設定，並套用至所有腳本。
...
<td nowrap>⚪ 前台</td>
<td>請先進入店長精選畫面，選擇「1-1」章節後，再按開始快捷鍵啟動。</td>
</tr>
<tr>
<td nowrap>自動釣魚</td>
<td nowrap>⚪ 前台 🟠 後台</td>
<td>請先進入釣魚畫面按下「開始釣魚」後，再按開始快捷鍵啟動腳本。</td>
</tr>
<tr>
<td nowrap>九百九十九夜 (赤龍古堡)</td>
<td nowrap>⚪ 前台</td>
<td>請先進入九百九十九夜副本，再按開始快捷鍵啟動鈕扣代幣循環。</td>
</tr>
</tbody>
</table>
...
透過工具列的設定按鈕，即可在 `F1` 至 `F12` 間設定所有腳本共用的開始與停止快捷鍵；預設為 `F1` 開始、`F2` 停止。
...
1. 將遊戲解析度設為 **1920 × 1080**，並參考下圖在赤龍古堡地圖放置四個標記。

![九百九十九夜地圖標記位置](assets/preview-999-1.png)

![九百九十九夜地圖標記圖示](scripts/999_nights/template/sample.png)

> 圖示由左至右分別為：門口、Boss 中心、折返路線 1、折返路線 2。

2. 進入「九百九十九夜」副本，使用前台模式啟動腳本，再按開始快捷鍵開始循環。
3. 腳本會操作滑鼠、`W`、`Shift`、`F` 與 `ESC`；執行期間請勿操作鍵盤滑鼠。
4. 如需停止，按停止快捷鍵回到待機。
...
**⚪ 前台模式**
按鍵會送至目前最上層視窗，請確認按下**開始快捷鍵**前已切回遊戲畫面。
